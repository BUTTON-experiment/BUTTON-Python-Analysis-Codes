...
split_filepath = str(sys.argv[1]).split("/")[:-2]
input_filepath = "/".join(split_filepath)
num_events=int(sys.argv[2])

## Reading PMT properties from PMTINFO
...
def LoopOverEventToGetMultiHitInfo(discretisedTimes, eventPMTIDs):
    ...
    multiHitDictionary = {  }
    uniquePMTIDs = np.unique(eventPMTIDs)
    for uniquePMTID in uniquePMTIDs:
        currentPMTIndexes = np.argwhere(eventPMTIDs == uniquePMTID) 
        currentPMTTimes = discretisedTimes[currentPMTIndexes]
        uniquePMTTimes, PMTTimesCounts = np.unique(currentPMTTimes, return_counts=True)
...
with uproot.open(sys.argv[1]) as file:#input_filepath+sys.argv[1]) as file: #CoincidenceStudy/10kEvents/coincidenceTest800Photons_lb.ntuple.root# "lightBallCalibration3_150Photons_120kEvents.ntuple.root"
    sysfile = sys.argv[1]
    startstring = 'coincidenceTest'
    endstring = 'Photons'
    get_numPhotons_str = sysfile[sysfile.find(startstring)+len(startstring):sysfile.rfind(endstring)]
    print ("************************** ",sys.argv[1])
    print(file.keys())
    ...
    print("mcPMTNPE sum:",np.sum(testVariable))
    
    
    fig,ax = plt.subplots(1, 1, figsize=[16,8])
    binEdges = np.arange(0,20,step=0.1)
    ...
    figPMTHits, axPMTHits = plt.subplots(1, 1, figsize=[16,8])
...
# #      print(file["T"])
    ...
    pmtHits = np.zeros(96)
    for i in range(96):
        pmtHits[i] += np.count_nonzero( output["mcPMTID"].array() == i)
    print(pmtHits)
    print(pmtHits.sum())
    
    barEdges = np.arange(0,96, step=1)
    figNPEDist, axNPEDist = plt.subplots(1, 1, figsize=[32,16])
    ...
    print(np.sum(output["mcparticlecount"].array()),"total number of photons in")
    print(np.amax(pmtHits),"number of hits in most-hit pmt")
    print(np.amin(pmtHits),"number of hits in least-hit pmt")
    ...
    timeSamples = np.arange(-80,220,step=2) # Samples
    ...
    for eventNumber in range(len(hitTimes)):
        ...
        discreteSampleLocations = np.searchsorted(timeSamples,currentHitTimes,side='right')-1
    ...
    np.save(input_filepath+"/coincidence/CoincidenceStudy_" + get_numPhotons_str + "_photons_"+str(int(num_events/1000.))+"kEvents_lb", coincidenceValues)

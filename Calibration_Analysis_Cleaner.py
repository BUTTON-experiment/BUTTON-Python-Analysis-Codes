...
def GaussianForFit(x, mean, sigma, amplitude):
    return amplitude * np.exp(-1 * ((x-mean)/(np.sqrt(2)*sigma))**2)
...
def TwoGaussiansForFit(x, mean1, sigma1, amplitude1, mean2, sigma2, amplitude2):
    ...
    gaussian1 = amplitude1 * np.exp(-1 * ((x-mean1)/(np.sqrt(2)*sigma1))**2)
    gaussian2 = amplitude2 * np.exp(-1 * ((x-mean2)/(np.sqrt(2)*sigma2))**2)
...
def FitGaussianIntegration(integrationsToFit, binEdges, currentHistXs, currentHistYs, centeredXs, centeredXsScaled, printTestHistogram):
    ...
    histogramMean = np.average(midpointsOfBins, weights=currentHistYs)
    # "The estimated variance is the weighted average of the squared difference from the mean:"
    histogramVariance = np.average((midpointsOfBins - histogramMean)**2, weights=currentHistYs)
    histogramStdDev = np.sqrt(histogramVariance)
    ...
    if printTestHistogram == True:
        testXs = np.arange(currentHistXs[0], currentHistXs[-1], step=(currentHistXs[1]-currentHistXs[0])/100)
        ...
        figInitial.savefig("single_gaussian_integration.jpg")
    ...
    if printTestHistogram == True:
        ...
        figFitted.savefig("single_gaussian_fit.jpg")
...
def FitTwoGaussianIntegration(integrationsToFit, binEdges, currentHistXs, currentHistYs, centeredXs, centeredXsScaled, printTestHistogram):
    ...
    if printTestHistogram == True:
        ...
        figInitial.savefig("double_gaussian_integration.jpg")
    ...
    lowerBounds = [-np.inf, 0, 0, -np.inf, 0, 0]
    #lowerBounds = [-np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf]
    upperBounds = [np.inf, np.inf, np.inf, np.inf, np.inf, np.inf]
    ...
    if printTestHistogram == True:
        ...
        figFitted.savefig("double_gaussian_fit.jpg")
    return currentFitValues, currentFitCovariance
...
def FitIntegrations(integrationsToFit, numBins, printTestHistogram = False, returnCovariance = False, fitType = "gaussian"):
    ...
    if len(integrationsToFit) >= 20: # want a baseline number of pmt Integrations to actually fit to
        ...
        integrationsToFit = np.abs(integrationsToFit)
        print(np.amin(integrationsToFit))
        
        minimumIntegrations = np.amin(integrationsToFit)
        maximumIntegrations = np.amax(integrationsToFit)
        ...
        binEdges = np.arange(binLowerBound, binUpperBound, step=IntegrationsDifference/numBins)
        

        # Create the histogram 
        currentHist = np.histogram(integrationsToFit, bins=binEdges)

        
        currentHistYs = currentHist[0]
        currentHistYs = currentHistYs.astype(np.float64)
        # maxHistY = np.amax(currentHistYs)
    ...
    if returnCovariance == True:
        perr = np.sqrt(np.diag(currentFitCovariance))
        ...
        print(perr)
        singleGaussianFitCovarianceCombined = np.sqrt( (perr[0])**2 + (perr[1])**2 + (perr[2])**2 )
...
def FitAdaptiveIntegration(integrationsToFit, binEdges, currentHistXs, currentHistYs, centeredXs, centeredXsScaled, printTestHistogram):
    ...
    perr = np.sqrt(np.diag(firstFitCovariance))
        # The covariance varies for the different parameters in the estimation
        # For a single gaussian I will have 3 parameters
        # Generally I only really care about the first gaussian anyway
        # There are 3 parameters then that I have to use to judge whether or not to try a two-gaussian fit
        ...
        # And then that way if even one of them is way off then it will sound the alarm
    #print(perr)
    combinedCovariance = np.sqrt( (perr[0])**2 + (perr[1])**2 + (perr[2])**2 )
...
def GetTruthValues(): # (analysisType, fitType):
    ...
    #     trueHeightSpread = 6.522136474 /1000 / np.sqrt(2) # divide by sqrt 2 to fix the mistake in the gaussian from fitting
...
def GetCombinedPMTPulseDataFromFile(numberOfPMTs, sourceType, fileName):
    ...
    #combinedPMTPulseData = np.empty(numberOfPMTs, dtype="object")
    # Create result array
    ...
    with uproot.open(fileName) as file: # "lightBallCalibration3_150Photons_120kEvents.ntuple.root"
        ...
        flat_indexes = np.concatenate([np.array(x) for x in pmtIDList])
        print("Concatenated indexes")
        flat_values = np.concatenate([np.array(x) for x in integrationsList])
...
def CalibrationAnalysis(fileName, makeSingleRunFigures = True):
    ...
    # fileName =
    fileIdentifier = fileName + ".ntuple.root" # just a file name
    ...
    if useSavedData == True:
        combinedPMTPulseData_df = pd.read_csv(fileName + "_" + source + "_savedPMTPulseData.csv", delimiter = ",")
    ...
    if saveCombinedData == True and useSavedData == False:
        ...
        df.to_csv(fileName + "_" + source + "_savedPMTPulseData.csv", index=None)
    ...
    fittedParameters = np.empty(numberOfPMTs, dtype="object")
    covariances = np.zeros(numberOfPMTs)
    ...
    if makeSingleRunFigures == True:
        ...
        pmtRelativeMeans = (np.array(pmtFittedMeans) - trueMeanHeight)/trueMeanHeight
...
def GetPMTHitCounts(fileName):
    ...
    with uproot.open(fileIdentifier) as file: # "lightBallCalibration3_150Photons_120kEvents.ntuple.root"
        ...
        uniquePMTs, pmtCounts = np.unique(flat_indexes, return_counts=True)
...
if __name__ == "__main__":
    ...
    pmtFittedSigmasList = []
    fileNames = [
        "../Testing_Daniel_Analysis_Codes/integrationTest_Results/integrationTest_1e4events"
        ]
    '''
    ...
    pmtFittedMeansList = np.array(pmtFittedMeansList)
    pmtFittedSigmasList = np.array(pmtFittedSigmasList)
    
    
    
    
    averagePMTFittedMeans = np.average(pmtFittedMeansList,axis=0)
    stdDevPMTFittedMeans = np.std(pmtFittedMeansList, axis=0)
    #averageMPTFittedStdDevs = np.average(pmtFittedSigmasList,axis=1)
    ...
    if getAverageHitCounts == True:
        averagePMTHitCounts = np.empty(numberOfPMTs,dtype=float)
        for fileName in fileNames:

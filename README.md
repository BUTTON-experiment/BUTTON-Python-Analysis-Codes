...
7. Open **CoincidenceStudy.py** and change the number of events (num_events) and pmtNumber if you want.
8. Run it by typing `python3 CoincidenceStudy.py`. Coincidence plot will be saved in the data directory with *coincidence_plot.png*.
...
- **Root_File_Reader.py** has been modified from Daniel's version due to the automation of the process. It now takes in two arguments just for the file path and the total number of events simulated, these are both populated automatically if you use **make_allcoincidence_macros.C** and run **Read_file_Reader.py** using **run_read_root_file.sh**.
- A lot of the file path modifications I made assume that **BUTTON-RAT2** and **BUTTON-Python-Analysis-Codes** are at the same directory level. If that is not true for you you will need to change paths in the code.
